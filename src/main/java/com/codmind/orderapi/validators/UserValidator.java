...
public class UserValidator {
    ...
    public static void signup(User user) {
        ...
        if(user.getUsername().length()>30) {
            throw new ValidateServiceException("El nombre de usuario es muy extenso (máx 30 caracteres)");
        }
        if(user.getPassword() == null || user.getPassword().isEmpty()) {
            throw new ValidateServiceException("El password de usuario es requerido");
        }
        if(user.getPassword().length()>30) {
            throw new ValidateServiceException("El password de usuario es muy extenso (máx 30 caracteres)");
        }
    }
}
